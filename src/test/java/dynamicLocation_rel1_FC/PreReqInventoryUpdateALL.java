...
public class PreReqInventoryUpdateALL extends BaseTest {
    ...
    @DescriptionProvider(slug = "Pre Req inventory update", description = "Pre requisite inventory update script ", author = "tushar")
    @Test(groups = {"preReqInventoryUpdateAll"})
    public void preReqInventoryUpdate() {
        ...
        int[] skuArray = fetchListOfSkusInPrimaryBin(fcId);
     ...
     * @return
     */
    public static int[] fetchListOfSkusInPrimaryBin(int fcId) {
        int numberSkuListInPrimaryLoc;
        String query = "select count(*) as number_of_sku from fc_sku_stock where " +
                "planogram_id = (select id from fc_planogram where fc_id = " + fcId + ") " +
                "and location_id=1 group by location_id;";
        System.out.println("Fetching the number of skus present in primary location from fc_sku_stock table\n " + query);
        ...
        if (jsonObject.getInt("numRows") == 0) {
            Assert.fail("No entry returned from table " + query);
        }
        numberSkuListInPrimaryLoc = jsonObject.getJSONArray("rows").getJSONObject(0).getInt("number_of_sku");
        int skuList[] = new int[numberSkuListInPrimaryLoc];
        System.out.println("Number of skus present in the list is : " + numberSkuListInPrimaryLoc);
        int offSet = 0;
        int num_of_records_to_fetch = 50;

        do {
            System.out.println("Current offset is " + offSet);
            query = "select sku_id,quantity,bin_id,bin_location,batch_id from fc_sku_stock " +
                    "where planogram_id=(select id from fc_planogram where fc_id = " + fcId + ") and " +
                    "location_id=1 order by id limit " + num_of_records_to_fetch + " OFFSET " + offSet + ";";
            ...
            if (numProdReturned < num_of_records_to_fetch) {
                System.out.println("Couldn't get the number of products requested, Number of product fetched is : " + numProdReturned);
            }


            for (int i = offSet, j = 0; j < numProdReturned; j++, i++) {
                skuList[i] = jsonObject.getJSONArray("rows").getJSONObject(j).getInt("sku_id");
            }
            offSet += num_of_records_to_fetch;
        } while ((offSet) < numberSkuListInPrimaryLoc);
