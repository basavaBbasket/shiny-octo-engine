...
public class OrderStatusUpdate extends BaseTest {
    ...
    @DescriptionProvider(author = "tushar",  description = "Verfies different response keys in Order Detail, List, Status and Tracking api",slug = "Checks Order Status API")
    ...
    public void checkOrderStausAtDifferentLevels()
    {
        ...
        String orderStatus = OrderUtils.getCurrentOrderStatus(Integer.parseInt(orderId), report);
        ...
        String checkOrderStatus = OrderUtils.getCurrentOrderStatus(Integer.parseInt(orderId), report);
        Assert.assertTrue(checkOrderStatus.equalsIgnoreCase("binned"), "Order status is not updated to binned " +
                "\n Current Order Status is: " + checkOrderStatus);
        report.log("Order status updated to binnned ", true);
        ...
        String queryForModLog = "select new_value from order_modificationlog where order_id="+orderId+" and order_status='packed';";
        ...
        String newVal = mod_log.getJsonArray("rows").getJsonObject(0).getString("new_value");
        String new_val_status = new JSONObject(newVal).getString("status");
        Assert.assertTrue(new_val_status.equalsIgnoreCase("binned"), "Order status is not modified to binned " +
                "\n Current Order Status is: " + new_val_status);
