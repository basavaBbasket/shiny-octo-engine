...
public class GetSupplierInformationTest extends BaseTest {
    ...
    @Test(groups = {"bbnow" , "regression","dl2","catalogfc","dl2-schema-validation"})
    public void orderTrackingApiTest(){
        ...
        supplierids.add(2);
        ArrayList<Integer> fcids=new ArrayList<>();
        fcids.add(236);
        ArrayList<Integer> mcids=new ArrayList<>();
        mcids.add(1);

        GetSupplierInformation getSupplierInformation= new GetSupplierInformation(xCaller,xEcId,ec,report);


        Response response = getSupplierInformation.getSupplierDetails("schema//catalog//supplier//supplier-information-200.json",supplierids,fcids,mcids);
