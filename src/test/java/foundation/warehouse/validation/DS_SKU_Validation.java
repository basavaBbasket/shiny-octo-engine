...
public class DS_SKU_Validation extends BaseTest {

    @DescriptionProvider(slug = "Path & Path-type validation of DS Sku", author = "Robin", description = "Path and path type validation of DS SKUs", authorEmail = "rajnish.robin@bigbasket.com")
    @Test(dataProvider = "data-provider", groups = {"bb2foundation", "bb2catalog"}, invocationCount = 1)
    public void DS_SKU_Validation_test(Map<String, Integer> data) throws Exception {
        ...
        String visibilityBody = catalogVisiblityAPIs.createCatalogVisiblityBody(sku_list, sa_list);
        Response visibilityResponse = catalogVisiblityAPIs.requestSKUPathInfo(visibilityBody);

        int path_id = catalogVisiblityAPIs.fetchPathIdFormResponse(visibilityResponse);
