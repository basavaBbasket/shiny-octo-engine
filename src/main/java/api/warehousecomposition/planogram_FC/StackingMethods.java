...
public class StackingMethods extends Helper {
    ...
    public static List<Integer> doStackingOnAlternateBinAndReturnAlternateBinIds(int fcId, int skuId, String userName, HashMap<String, Object> grnCreateStackingJobBody, Map<String, String> cookie, IReport report)
    {
        List<Integer> binIdList = new ArrayList<>(); // index 0: binId , index 1: alternate bin Id
        IQApp app = new IQApp(report);
        app.cookie.updateCookie(cookie);
        ...
        int jobId = fetchJobId(response, report);
        int binId = Integer.parseInt(response.path("path[0].bin_id").toString());
        binIdList.add(binId);

        DynamicLocationGeneralMethods.updateMaxAlternateBins(2, report);

        String alternateBinApiBody = prepareGrnStackingAlternateBinApiBody(skuId, Integer.parseInt(grnCreateStackingJobBody.get("total_quantity").toString()), report);
        Response alternateBinApiResponse = app.stackingAlternateBinAPI(fcId, jobId, alternateBinApiBody);
        int len = Integer.parseInt(alternateBinApiResponse.path("path.size()").toString());

        for(int i =0; i< len; i++)
        {
            binIdList.add(Integer.parseInt(alternateBinApiResponse.path("path[0].bin_id").toString()));
