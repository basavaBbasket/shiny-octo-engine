...
public class InventoryMethods extends Helper {
    ...
    public static boolean isPrimaryBinMappedForSku(Map<String, String> adminCookie, int skuId, int fcId, IReport report) {
               return isPrimaryBinMappedForSku(adminCookie,skuId,fcId,report,true);
    }
     ...
     * @return return boolean value
     */
    public static boolean isPrimaryBinMappedForSku(Map<String, String> adminCookie, int skuId, int fcId, IReport report, boolean logToReport) {
        IQApp app = new IQApp(report);
        app.cookie.updateCookie(adminCookie);
        Response response = app.stockInfoAPI(fcId, skuId, logToReport);
        if (response.path("primary_qoh.bins_qoh") == null) {
            if (logToReport) {
                report.log("Primary bin is not mapped in sku-bin mapping for the sku " + skuId, true);
            }
            System.out.println("Primary bin is not mapped in sku-bin mapping for the sku " + skuId);
            return false;
        } else {
            int totalQty = response.path("primary_qoh.total_qoh");
            if (totalQty <= 0) {
                if (logToReport) {
                    report.log("Primary bin is mapped in sku-bin mapping for the sku " + skuId + " " +
                            "but qty is  " + totalQty, true);
                }
                System.out.println("Primary bin is mapped in sku-bin mapping for the sku " + skuId + " " +
                        "but qty is  " + totalQty);
