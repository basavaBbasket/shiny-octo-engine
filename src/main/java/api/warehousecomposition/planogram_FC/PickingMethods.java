...
public class PickingMethods extends Helper {
     ...
     * @return
     */
    public static JSONObject fetchContainerLabelInUse(IReport report,int fcId,Map<String, String> adminCookie) {
        ...
        for(int i=0; i<=1;i++) {
            ...
            if (jsonObject.getInt("numRows") == 0) {
                report.log("None of the containers with in-use status" + query, true);
                linkBagWithExistingOrder(report, fcId, adminCookie);
            }
            else{
                break;
            }
    ...
    private static void linkBagWithExistingOrder(IReport report, int fcId, Map<String,String> adminCookie) {
        String query = "select order_id from picking_request where  DATE(created_on)=DATE(NOW() - INTERVAL 1 DAY) limit 1;";
        JSONObject jsonObject = new JSONObject(PickingPlatformQueryExecuteWrapper.executeMicroserviceDataBaseQuery(query));
        if (jsonObject.getInt("numRows") == 0) {
            Assert.fail("No open order are avialable for container assignment");
        }
        int orderId = jsonObject.getJSONArray("rows").getJSONObject(0).getInt("order_id");
        String containerLabel = fetchAvialableContainer(report);
        orderBagLinking(fcId, orderId, containerLabel, adminCookie, false, report);

    }

    private static String fetchAvialableContainer(IReport report) {

        String query = "select container_label from container where status=\"Available\" limit 1;";
        report.log("Fetching container label which are Avialable\n " + query, true);
        JSONObject jsonObject = new JSONObject(PickingPlatformQueryExecuteWrapper.executeMicroserviceDataBaseQuery(query));
