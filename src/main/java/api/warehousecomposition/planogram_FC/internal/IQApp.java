...
public class IQApp extends WebSettings implements Endpoints {
    ...
    public Response jobAssignment(int fcId, HashMap<String, Object> body) {
        ...
        if(!(new JsonObject(response.asString()).getJsonArray("picking").isEmpty())) {
            report.log("Verifying whether sku_location_info key is empty in job assignment api response",true);
            ...
            report.log("Value for picking key is empty in Job assignment api response",true);
        }
