...
public class OrderStatusAPI extends WebSettings implements Endpoints{
    ...
    public Response changeOrderStatus(String status, String orderID, List<Integer> sku) {
        ...
        JsonArray arr = new JsonArray().add(new JsonObject().put("id",orderID).put("status",status).put("timestamp",new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
                .format(new DateTime(new Date()).toDate())).put("items",skuArr));
