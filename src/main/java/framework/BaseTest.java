...
public class BaseTest extends WebSettings {
    ...
    public AutomationReport getInitializedhubopsReport(Class<?> testClass, boolean setupDriver) {
        AutomationReport report = obj.getInitializedReport(testClass);
        if (setupDriver) {
            WebDriver driver = WebDriverUtilities.gethubopsWebDriver();
            WebDriverUtilities.maximizeDriver(driver);
            report.setWebDriver(driver);
        }
        return report;
    }
    public AutomationReport getInitializedhubopsReport(Class<?> testClass, boolean setupDriver,Class<?>... class1) {
        AutomationReport report = obj.getInitializedReport(testClass,class1);
        if (setupDriver) {
            WebDriver driver = WebDriverUtilities.gethubopsWebDriver();
            WebDriverUtilities.maximizeDriver(driver);
            report.setWebDriver(driver);
