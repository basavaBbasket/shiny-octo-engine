...
public class SuiteListener extends Settings implements ITestListener, ISuiteListener, IExecutionListener, IAlterSuiteListener {
    ...
    public void verifyProductionEligibility() throws Exception {
        List<String> eligibleProdSuites = Arrays.asList(Settings.config.getProperty("prodTags").split(","));
        List<String> suitesToExecute = Arrays.asList(suiteName.split(","));
        if (isProdEnvironment(serverName)){
            if (!new HashSet<>(eligibleProdSuites).containsAll(suitesToExecute)){
                logger.error("CRITICAL. Cannot run all suites on production environment.");
                logger.error("Eligible suites for production : " + eligibleProdSuites);
                logger.error("Suites to run: " + suitesToExecute);
                throw new Exception("Please do not try to test in production server");
            } else {
                logger.info("Triggering " + suitesToExecute + "in production");
            }
        }
    }
